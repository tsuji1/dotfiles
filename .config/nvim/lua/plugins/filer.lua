return {
	{
		...
		config = function()
			...
			-- これはグローバルでOK（トグル起動用）
			--
			-- -- NvimTree からエディタへフォーカスを戻すショートカット
			vim.keymap.set("n", "<leader>eo", "<cmd>wincmd p<CR>", { desc = "Focus back to editor" })
			vim.keymap.set("n", "<leader>ee", "<cmd>NvimTreeToggle<CR>", { desc = "Toggle file explorer" })
			vim.keymap.set("n", "<leader>ef", "<cmd>NvimTreeFocus<CR>", { desc = "NvimTreeFocus" })
		end,
	},
}
